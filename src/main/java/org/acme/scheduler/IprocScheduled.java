...
public class IprocScheduled {
    ...
    public void send_tdr_iproc_skt(){
        ...
        if(tdrEprocStatus.getStatus_eproc().equals(StatusEprocEnum.ON)){
            // System.out.println("status iproc skt in = "+tdrEprocStatus.getStatus_eproc());
            try {
                LOG.info("Send Data Eproc SKT begin");
                List<TdrIdentitasVendorEntity> tdrIdentitasVendorEntity = tdrIdentitasVendor.find("where is_minio_identitas_vendor = ?1", 1).page(0, 50).list();
                for (TdrIdentitasVendorEntity p : tdrIdentitasVendorEntity) {
                    if(p.getId_identitas() != null){
                        TdrPengajuanVendorEntity tdrPengajuan = tdrPengajuanVendor.find("where id_identitas = ?1 and is_eproc is null ", p.getId_identitas()).firstResult();
                        try {
    ...
    public void send_tdr_iproc_nonskt(){
        ...
        TdrSendFileEprocStatusEntity tdrEprocStatus = TdrSendFileEprocStatusEntity.find("nama_jobs = ?1", mainJobsApps).firstResult();
        // System.out.println("status iproc nonskt = "+tdrEprocStatus.getStatus_eproc());
        LOG.info("Status Scheduler iproc nonskt = "+tdrEprocStatus.getStatus_eproc());
        if(tdrEprocStatus.getStatus_eproc().equals(StatusEprocEnum.ON)){
            // System.out.println("status iproc nonskt in = "+tdrEprocStatus.getStatus_eproc());
            
            try {
                LOG.info("Send Data Eproc Non SKT begin");
                // List<TdrIdentitasVendorNonSktEntity> tdrIdentitasVendorEntity = TdrIdentitasVendorNonSktEntity.find("where is_minio_identitas_vendor = ?1", 1).page(0, 10).list();
                List<TdrPengajuanVendorNonSktEntity> tdrPengajuanList = TdrPengajuanVendorNonSktEntity.find("WHERE is_eproc is null ORDER BY id_identitas DESC").page(0, 50).list();
                for (TdrPengajuanVendorNonSktEntity tdrPengajuan : tdrPengajuanList) {
                    TdrIdentitasVendorNonSktEntity p = TdrIdentitasVendorNonSktEntity.find("where is_minio_identitas_vendor = ?1 AND id_identitas = ?2", 1, tdrPengajuan.getId_identitas()).firstResult();
                    // System.out.println("identitas = "+p.getId_identitas());
                    try {
                        
                        if(p != null && p.getId_identitas() != null){

                            // TdrPengajuanVendorNonSktEntity tdrPengajuan = TdrPengajuanVendorNonSktEntity.find("where id_identitas = ?1", p.getId_identitas()).firstResult();
                            
                            // System.out.println("pengajuan = "+tdrPengajuan);
                            if(tdrPengajuan != null && tdrPengajuan.getId_pengajuan() != null){
                        ...
                        LOG.error("id_identitas = "+p.getId_identitas()+" error = "+e.getMessage());
                        // TODO: handle exception
                    }
